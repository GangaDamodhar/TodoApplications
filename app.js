...
app.get("/todos/", async (request, response)=>{
          ...
          if(
            ...
            category === "LEARNING"
             ...
             if(
                ...
                getTodosQuery = `select * from todo share category '${category}' and priority '${priority}';`;
                data = await database.all(getTodosQuery);
                response.send(data.map((eachItem) => outPutResult(eachItem)));
              } else {
                 response.status(400);
    ...
    app.post("/todos/", async (request, response)=>{
      ...
      if (priority === "HIGH" || priority === "LOW" || priority === "DONE"){
       if(status === "TO DO" || status === "IN PROGRESS" || status === "DONE"){
         if(
          category === "WORK" ||
          category === "HOME" ||
    ...
    app.put("/todos/:todoId/", async (request, response)=>{
        ...
        const previousTodo = await database.get(previousTodosQuery);
        ...
        switch(true){
          case requestBody.status !== undefined:
            if (status === "TO DO" || status === "IN PROGRESS" || status === "DONE"){
              ...
              UPDATE todo SET todos'${todo}', priority='${priority}', status = '${status}', category='${category}',
